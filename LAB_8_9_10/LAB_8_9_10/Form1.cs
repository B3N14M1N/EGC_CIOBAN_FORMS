...
namespace OpenTK_winforms_z02 {
    public partial class Form1 : Form {
        ...
        private float[] light_1_pos = { 0,100f,0f,1f};
        private Keys movLightUp = Keys.ShiftKey;
        private Keys movLightDown = Keys.Control;
        private Keys movLightLeft = Keys.A;
        private Keys movLightRight = Keys.D;
        private Keys movLightForward = Keys.W;
        private Keys movLightBackwards = Keys.S;
        /*************************************************************************************************************************************************************/
        //L9
        private bool statusOBJTex = false;
        /*************************************************************************************************************************************************************/
        ...
        private void GlControl1_Paint(object sender, PaintEventArgs e) {
            ...
            /*************************************************************************************************************************************************************/
            // L8
            GL.Light(LightName.Light1, LightParameter.Ambient, valuesAmbient0);
            GL.Light(LightName.Light1, LightParameter.Diffuse, valuesDiffuse0);
            GL.Light(LightName.Light1, LightParameter.Specular, valuesSpecular0);
            GL.Light(LightName.Light1, LightParameter.Position, light_1_pos);
        ...
        private void GlControl1_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (lightON && lightON_1)
            {
                if (e.KeyCode == movLightUp)
                {
                    light_1_pos[1] += 10f * (float)numericUpDownLightSensivity.Value;
                }
                if (e.KeyCode == movLightDown)
                {
                    light_1_pos[1] -= 10f * (float)numericUpDownLightSensivity.Value;
                }
                if (e.KeyCode == movLightForward)
                {
                    light_1_pos[2] = 10f * (float)numericUpDownLightSensivity.Value;
                }
                if (e.KeyCode == movLightBackwards)
                {
                    light_1_pos[2] -= 10f * (float)numericUpDownLightSensivity.Value;
                }
                if (e.KeyCode == movLightLeft)
                {
                    light_1_pos[0] -= 10f * (float)numericUpDownLightSensivity.Value;
                }
                if (e.KeyCode == movLightRight)
                {
                    light_1_pos[0] += 10f * (float)numericUpDownLightSensivity.Value;
                }
                GlControl1.Invalidate();
        ...
        /*************************************************************************************************************************************************************/
        //L9
        private void btnTexOBJ_Click(object sender, EventArgs e)
        {
            resetTexCubes();
            statusOBJTex = true;
            GlControl1.Invalidate();
        }
        private void btnResetObj_Click(object sender, EventArgs e)
        {
            resetTexCubes();
            statusOBJTex = false;
            numericUpDownTeapotScale.Value = numericUpDownTeapotScale.Minimum;
            GlControl1.Invalidate();
        }
        private void DeseneazaOBJ_Tex()
        {
            if (brick == true)
            {
                GL.BindTexture(TextureTarget.Texture2D, textures[0]);
            ...
            MeshData mesh = new LoadOBJ().ReadOBJ("teapot.obj");

            GL.PushMatrix();
            GL.Scale((float)numericUpDownTeapotScale.Value,
                (float)numericUpDownTeapotScale.Value,
                (float)numericUpDownTeapotScale.Value);
            ...
            {
                GL.TexCoord3(mesh.vertex[(int)mesh.faces[i].X - 1]/5);
                GL.Vertex3(mesh.vertex[(int)mesh.faces[i].X - 1]);
                GL.TexCoord3(mesh.vertex[(int)mesh.faces[i].Y - 1]/5);
                GL.Vertex3(mesh.vertex[(int)mesh.faces[i].Y - 1]);
                GL.TexCoord3(mesh.vertex[(int)mesh.faces[i].Z - 1]/5);
                GL.Vertex3(mesh.vertex[(int)mesh.faces[i].Z - 1]);
            }
            GL.End();
            GL.PopMatrix();
        }
        private void numericUpDownTeapotScale_ValueChanged(object sender, EventArgs e)
        {
            GlControl1.Invalidate();
        }
        /*************************************************************************************************************************************************************/
